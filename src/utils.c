...
void copy_to_clipboard(const char *password) {
    ...
    printf("INFO: Clipboard operacija u toku...\n");
    printf("INFO: Lozinka se kopira na sistemski clipboard\n");
 ...
 * @param password - Lozinka za čuvanje
 * @param filename - Naziv datoteke (default: PASSWORD_FILE)
 * @return 0 ako je uspešno, -1 ako ima greške
 */
int save_password_to_file(const char *password, const char *filename) {
    ...
    // Koristi default filename ako nije prosleđen
    ...
    // Dobij trenutni time za timestamp
...
/**
 * Funkcija za čitanje yes/no izbora korisnika
 * @return 'y'/'Y' za yes, 'n'/'N' za no, ili '0' ako ima greške
 */
char get_yes_no_choice() {
    ...
    // Čitaj jedan karakter
    if (scanf("%c", &choice) != 1) {
        fprintf(stderr, "Greska pri citanju unosa!\n");
        return '0';
    }

    // Očisti ostatak input buffer-a
    while (getchar() != '\n');

    // Konvertuj u lowercase za lakšu proveru
 ...
 * @param password - Generisana lozinka
 */
